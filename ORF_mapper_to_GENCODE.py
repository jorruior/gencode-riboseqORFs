...
def main():
	...
	parser.add_option("-b","--input_bed",action="store",dest="orfs_bed_file",help="(Required) File with 1-based BED coordinates of all translated candidate ORFs. ORF names should match in both fasta and bed files. If -m is activated, the BED coordinates will be written into this file.")
	...
	parser.add_option("-L","--max_len_cutoff",action="store",type=int,dest="max_len_cutoff",default="999999999999",help="Maximum ORF length threshold in amino acids (without stop codon). default=inf")
	...
	if calculate_coordinates != "no":
		functions.make_bed(orfs_fa,transcriptome_fa,gtf,len_cutoff,max_len_cutoff,calculate_coordinates,orfs_bed_file,out_name,mult)
	...
	functions.write_output(orfs_fa_file,orfs_bed_file,candidates,exc,variants,variants_names,datasets,appris,supp,gtf,transcriptome_fa,second_names,len_cutoff,max_len_cutoff,col_thr,total_studies,other_overlaps,psites_bed_file,folder,out_name,method,seed)
