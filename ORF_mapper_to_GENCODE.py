...
def main():
	...
	parser.add_option("-d","--input_dir",action="store",dest="folder",help="Directory with required Ensembl files (transcriptome gft and fasta, proteome fasta, tab file with APPRIS and TSL support, protein psites generated by 'calculate_frame_bed.py'. Files are already available for Ens101 and the latest GENCODE version.")
	parser.add_option("-f","--input_fasta",action="store",dest="orfs_fa_file",help="File with all translated candidate ORFs. A FASTA can be generated from a BED file using the script 'bed1_to_fasta.sh'")
	parser.add_option("-b","--input_bed",action="store",dest="orfs_bed_file",help="File with 1-based BED coordinates of all translated candidate ORFs. ORF names should match in both fasta and bed files. If -m is activated, the BED file will be written")
	...
	if calculate_coordinates != "no":
		functions.make_bed(orfs_fa,transcriptome_fa,gtf,len_cutoff,calculate_coordinates,orfs_bed_file)
	...
	functions.write_output(orfs_bed_file,candidates,exc,variants,variants_names,datasets,appris,supp,gtf,transcriptome_fa,second_names,len_cutoff,col_thr,total_studies,other_overlaps,psites_bed_file,folder)
